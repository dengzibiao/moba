...
namespace Pathfinding.Serialization
{
	public class BoundsConverter : BaseConverter {
		public override bool CanConvert (Type type) {
			return type == typeof(Bounds);
		}
		...
		public override object ReadJson (JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer) {
			Bounds b = new Bounds();
			b.center = new Vector3(ReadFloat(reader),ReadFloat(reader),ReadFloat(reader));
			b.extents = new Vector3(ReadFloat(reader),ReadFloat(reader),ReadFloat(reader));
			return b;
		}
		...
		public override void WriteJson (JsonWriter writer, object value, JsonSerializer serializer) {
			...
			Write(writer,"cx",b.center.x);
			Write(writer,"cy",b.center.y);
			Write(writer,"cz",b.center.z);
			Write(writer,"ex",b.extents.x);
			Write(writer,"ey",b.extents.y);
			Write(writer,"ez",b.extents.z);
	...
	public abstract class BaseConverter : JsonConverter
	{
		...
		public void Write (JsonWriter writer, string label, object value) {
			writer.WritePropertyName(label);
			writer.WriteValue(value);
		}
	}
	
	public class VectorConverter : BaseConverter
	{
		public override bool CanConvert (Type type) {
			return type == typeof(Vector2) || type==typeof(Vector3)||type==typeof(Vector4);
		}
		...
		public override object ReadJson (JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer) {
			...
			if (objectType == typeof(Vector2)) {
				existingValue = new Vector2(ReadFloat(),ReadFloat());
			} else if (objectType == typeof(Vector3)) {
				existingValue = new Vector3(ReadFloat(),ReadFloat(),ReadFloat());
			} else if (objectType == typeof(Vector4)) {
				existingValue = new Vector4(ReadFloat(),ReadFloat(),ReadFloat(),ReadFloat());
			} else {
				throw new System.NotImplementedException ("Can only read Vector2,3,4. Not objects of type "+objectType);
			}
			reader.Read();
