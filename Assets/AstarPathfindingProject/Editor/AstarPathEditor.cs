...
public class AstarPathEditor : Editor {
	...
	[System.ObsoleteAttribute ("Use GetURL () instead")]
		public static string DownloadLink = "http://u3d.as/content/aron-granberg/a-pathfinding-project-pro/2h6";
...
#region Styles
	 ...
	 * \astarpro */
	public Dictionary<string,DefineObject> defines;
	...
	public void DrawOptimizationSettings () {
		...
		if (GUILayoutx.DrawID ("optimization")) {
			...
			GUILayout.Label ("Using C# pre-processor directives, performance and memory usage can be improved by disabling features that you don't use in the project.\nEvery change to these settings requires recompiling the scripts and the settings themselves are saved in the script source files as commented or uncommented #define statements",helpBox);
			...
			foreach (KeyValuePair<string,DefineObject> define in defines) {
				...
				GUIContent label = new GUIContent (ObjectNames.NicifyVariableName (define.Value.name),define.Value.brief+"\nDefined in:\n"+define.Value.files);
				
				if (define.Value.enumValues == null) {
					define.Value.enabled = EditorGUILayout.Toggle (label,define.Value.enabled);
				} else {
					/*int i = 0;
					...
					define.Value.selectedEnum = EditorGUILayout.Popup (label.text,define.Value.selectedEnum,define.Value.enumValues);
					
					//The define is enabled if it is not zero (zero == "Disabled")
					define.Value.enabled = define.Value.selectedEnum != 0;
				}
				
				GUILayout.Label (define.Value.brief+"\nDefined in: "+define.Value.files,helpBox);//,EditorStyles.miniLabel);
				if (define.Value.inconsistent) {
