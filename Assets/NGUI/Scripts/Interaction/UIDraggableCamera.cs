...
public class UIDraggableCamera : MonoBehaviour
{
    ...
    Vector3 CalculateConstrainOffset ()
    {
        ...
        Debug.LogError("minRect " + minRect + " maxRect " + maxRect + " bottomLeft " + bottomLeft + " topRight " + topRight);
    ...
    public void Drag ( Vector2 delta, Transform player = null )
    {
        ...
        float xPos = 0;
        float zPos = 0;
        mTrans.localPosition += Quaternion.Euler(0f, mRotY, 0f) * new Vector3(offset.x, 0f, offset.y);
        xPos = mTrans.localPosition.x > mXmax ? mXmax : mTrans.localPosition.x;
        xPos = xPos < mXmin ? mXmin : xPos;
        zPos = mTrans.localPosition.z > mZmax ? mZmax : mTrans.localPosition.z;
        zPos = zPos < mZmin ? mZmin : zPos;

        mTrans.localPosition = new Vector3(xPos, mTrans.localPosition.y, zPos);
        // Adjust the momentum
    ...
    float mXmax, mXmin, mZmax, mZmin, mRotY;
    public void SetDragBounds ( float Xmax, float Xmin, float Zmax, float Zmin, float rotY = -45)
    {
        mXmax = Xmax;
        mXmin = Xmin;
        mZmax = Zmax;
